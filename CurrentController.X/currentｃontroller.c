...
static char CMD_Reg[CMD_ADDR_MAX];
...
int CMD_Execute(const char *cmd){
    ...
    if(cnt > 1){
        if(ParseByte(buf[1],&addr) != 0 || addr >= CMD_ADDR_MAX){
            printf("%s%s",CMD_NOK_TEXT,CMD_LINE_ENCODE);
            return -1;
        }
    }
    
    if(cnt == 3 && strcmp(buf[0],CMD_SET_OPERATION) == 0){
        if(ParseByte(buf[2],&val) != 0){
            printf("%s%s",CMD_NOK_TEXT,CMD_LINE_ENCODE);
            return -1;
        }
        CMD_Reg[addr] = val;
    ...
    if(cnt == 2 && strcmp(buf[0],CMD_GET_OPERATION) == 0){
